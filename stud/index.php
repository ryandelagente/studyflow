...
// /index.php (Updated and Fully Dynamic)
...
// --- INITIALIZATION ---
$db_error = null;
// --- SAAS FIX: Use tenant_id from session for all queries ---
// studyflow/stud/index.php

// --- SAAS FIX: Use null coalescing to prevent the warning ---
$user_id = $_SESSION["id"]; 
$tenant_id = $_SESSION["tenant_id"] ?? 0;
$user_role = $_SESSION["role"] ?? 'user';

// OPTIONAL: If it's an admin, you might want to redirect them to the admin dashboard
...
if (!isset($link) || $link === false) {
    ...
    // 1. Fetch Task Statistics (Filtered by tenant_id)
    $sql = "SELECT COUNT(id) AS total, SUM(IF(is_completed = 1, 1, 0)) AS completed FROM tasks WHERE tenant_id = ?";
    ...
    // 2. Fetch Study Goal Statistics (Filtered by tenant_id)
    $sql = "SELECT COUNT(id) AS total, SUM(IF(is_completed = 1, 1, 0)) AS completed FROM study_goals WHERE tenant_id = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "i", $tenant_id);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
    ...
    // 3. Fetch Assignment Statistics (Filtered by tenant_id)
    $sql = "SELECT COUNT(id) AS total, SUM(IF(status = 'FINISHED', 1, 0)) AS completed FROM assignments WHERE tenant_id = ?";
     if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "i", $tenant_id);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
    ...
    // 4. Fetch Recent Incomplete Tasks (Filtered by tenant_id)
    $sql = "SELECT title, priority FROM tasks WHERE tenant_id = ? AND is_completed = 0 ORDER BY created_at DESC LIMIT 4";
    if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "i", $tenant_id);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
    ...
    // 5. Fetch Recent Pending Assignments (Filtered by tenant_id)
    $sql = "SELECT title, course, status FROM assignments WHERE tenant_id = ? AND status != 'FINISHED' ORDER BY end_date ASC LIMIT 3";
    if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "i", $tenant_id);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
    ...
    <?php else: ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white p-6 rounded-lg shadow col-span-1 md:col-span-2">
                ...
                <p class="text-3xl font-bold">138<span class="text-lg text-gray-500">m</span></p>
