...
if ($link && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['save_note'])) {
        ...
        if (!empty($title)) {
            if ($note_id_to_save > 0) { // This is an UPDATE
                $sql = "UPDATE notes SET title = ?, content = ? WHERE id = ? AND tenant_id = ?";
                if ($stmt = mysqli_prepare($link, $sql)) {
                    mysqli_stmt_bind_param($stmt, "ssii", $title, $content, $note_id_to_save, $tenant_id);
                    if (mysqli_stmt_execute($stmt)) {
    ...
    if (isset($_POST['delete_note'])) {
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            mysqli_stmt_bind_param($stmt, "ii", $note_id_to_delete, $tenant_id);
            if (mysqli_stmt_execute($stmt)) {
...
if (isset($_GET['success'])) {
    switch ($_GET['success']) {
        case 1: $success_message = "Note created successfully."; break;
        case 2: $success_message = "Note updated successfully."; break;
        case 3: $success_message = "Note deleted successfully."; break;
    }
}
...
if ($link) {
    if (($action === 'edit' || $action === 'create') && $note_id > 0) {
        // Fetch a single note for the editor (for the current tenant)
        $sql = "SELECT id, title, content FROM notes WHERE id = ? AND tenant_id = ?";
        if ($stmt = mysqli_prepare($link, $sql)) {
            mysqli_stmt_bind_param($stmt, "ii", $note_id, $tenant_id);
            mysqli_stmt_execute($stmt);
            $result = mysqli_stmt_get_result($stmt);
            $note_to_edit = mysqli_fetch_assoc($result);
            if (!$note_to_edit && $action === 'edit') { 
                header("Location: notes.php");
                exit();
            }
        }
    } elseif ($action === 'list') {
        // Fetch all notes for the list view (for the current tenant)
        $sql = "SELECT id, title, content, updated_at FROM notes WHERE tenant_id = ? ORDER BY updated_at DESC";
        if ($stmt = mysqli_prepare($link, $sql)) {
        ...
        aiForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (tinymce.activeEditor) {
                    tinymce.activeEditor.insertContent(data.reply);
                }
