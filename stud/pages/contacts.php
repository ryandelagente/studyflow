...
if ($link && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['update_contact'])) {
        ...
        $sql = "UPDATE contacts SET first_name=?, last_name=?, title=?, email=?, phone=?, address=?, city=?, state=?, zip=?, notes=? WHERE id=? AND tenant_id=?";
        if ($stmt = mysqli_prepare($link, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssssssssssii", $first_name, $last_name, $title, $email, $phone, $address, $city, $state, $zip, $notes, $contact_id, $tenant_id);
    ...
    if (isset($_POST['delete_contact'])) {
        ...
        $sql = "DELETE FROM contacts WHERE id = ? AND tenant_id = ?";
        if ($stmt = mysqli_prepare($link, $sql)) {
            mysqli_stmt_bind_param($stmt, "ii", $contact_id_to_delete, $tenant_id);
...
if (isset($_GET['success'])) { 
    switch ($_GET['success']) {
        case 1: $success_message = "Contact added successfully."; break;
        case 2: $success_message = "Contact updated successfully."; break;
        case 3: $success_message = "Contact deleted successfully."; break;
    }
}
...
if ($link) {
    if ($action === 'edit' && $contact_id > 0) {
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            mysqli_stmt_bind_param($stmt, "ii", $contact_id, $tenant_id);
        ...
        $sql = "SELECT * FROM contacts WHERE tenant_id = ? ORDER BY first_name ASC";
        if ($stmt = mysqli_prepare($link, $sql)) {
                    ...
                    <?php else: ?>
                        ...
                        <?php foreach ($contacts as $contact): ?>
                            <tr class="border-t hover:bg-gray-50">
                                ...
                                <td class="p-3 text-gray-600"><?php echo htmlspecialchars($_SESSION['username']); ?></td>
                                <td class="p-3 text-gray-600"><?php echo htmlspecialchars($contact['email']); ?></td>
                                    ...
                                    <div class="relative inline-block text-left">
                                        ...
                                        <div id="menu-<?php echo $contact['id']; ?>" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 hidden z-10">
                                            <div class="py-1">
                                                ...
                                                <form method="POST" action="contacts.php" onsubmit="return confirm('Are you sure you want to delete this contact?');" class="w-full">
